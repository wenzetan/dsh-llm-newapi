...
var NewApiAdapter = class extends LlmAdapter {
  ...
  async fetchModelsDevParams(request, signal) {
    ...
    try {
      ...
      throw new LlmError4("models.dev catalog fetch failed", "TRANSPORT", { cause: error });
    } finally {
...
function apply(ctx, config) {
  ...
  options();
  const resolveApiKey = async (connection2) => {
    const ref = connection2.apiKeyRef;
  ...
  if (connection !== void 0) {
    ctx.effect(() => connection.rpc.handle(
      ...
      (endpoint, payload, signal) => {
        ...
        return adapter.fetchModelsDevParams(request, signal).then((value) => ({ ok: true, value }));
      },
      { authority: "loopback" }
