  ...
  const resolveApiKey = async (connection) => {
    ...
    if (credentials !== void 0) {
      ...
      if (ambient !== void 0 && ambient.value.length > 0) {
        return assertUsableApiKey2(ambient.value, PKG, ref);
      }
    }
    throw new LlmError5(
      `${PKG}: no API key for provider route "${PROVIDER}"; store ${ref} through the credentials service (the web Models page writes it), or export ${ref} in the launching environment`,
      "MISSING_CREDENTIAL"
    );
  };
