  ...
  for await (const payload of payloads) {
    ...
    for (const choice of chunk.choices ?? []) {
      ...
      for (const call of delta?.tool_calls ?? []) {
        ...
        if (typeof name2 === "string" && name2.length > 0) block.name = name2;
...
if (!isSupportedHostVersion(hostLlmVersion)) {
  throw new Error(
    `dsh-llm-newapi requires dsh >= ${MINIMUM_DSH_VERSION} (host ships @deepseek-ai/dsh-llm ${hostLlmVersion}); upgrade the host: npm install -g @deepseek-ai/dsh@${MINIMUM_DSH_VERSION}`
  );
}
